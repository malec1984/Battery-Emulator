...
void MebBattery::
  ...
  memcpy(datalayer_battery->status.cell_voltages_mV, cellvoltages, datalayer.battery.info.number_of_cells * sizeof(uint16_t));
...
void MebBattery::handle_incoming_can_frame(CAN_frame rx_frame) {
  ...
  switch (rx_frame.ID) {
    ...
    case BMS_CMC_04:  // BMS Temperature and cellvoltages - 180ms
      ...
      switch (mux) {
        case 0:  // Temperatures 1-56. Value is 0xFD if sensor not present
          ...
          break;
        /*
        Broadcast cellvoltages are currently disabled, since they're not in use for MEB.
    ...
    case ISO_Hybrid_01_Resp_FD:           // Reply from battery
      if (rx_frame.data.u8[0] == 0x10) {  // ISOTP header (Single FRAME)
        transmit_can_frame(&ISOTP_ACK_FRAME);
      }
      if (rx_frame.DLC == 8) {
...
void MebBattery::transmit_can(unsigned long currentMillis) {
  ...
  if (currentMillis - previousMillis200ms >= INTERVAL_200_MS) {
    ...
    switch (poll_pid) {
      case PID_SOC:
        UDS_READ_DID_FRAME.data.u8[2] = (uint8_t)(PID_SOC >> 8);  // High byte
        UDS_READ_DID_FRAME.data.u8[3] = (uint8_t)PID_SOC;         // Low byte
        poll_pid = PID_VOLTAGE;
        break;
      case PID_VOLTAGE:
        UDS_READ_DID_FRAME.data.u8[2] = (uint8_t)(PID_VOLTAGE >> 8);
        UDS_READ_DID_FRAME.data.u8[3] = (uint8_t)PID_VOLTAGE;
        poll_pid = PID_CURRENT;
        break;
      case PID_CURRENT:
        UDS_READ_DID_FRAME.data.u8[2] = (uint8_t)(PID_CURRENT >> 8);
        UDS_READ_DID_FRAME.data.u8[3] = (uint8_t)PID_CURRENT;
        poll_pid = PID_MAX_TEMP;
        break;
      case PID_MAX_TEMP:
        UDS_READ_DID_FRAME.data.u8[2] = (uint8_t)(PID_MAX_TEMP >> 8);
        UDS_READ_DID_FRAME.data.u8[3] = (uint8_t)PID_MAX_TEMP;
        poll_pid = PID_MIN_TEMP;
        break;
      case PID_MIN_TEMP:
        UDS_READ_DID_FRAME.data.u8[2] = (uint8_t)(PID_MIN_TEMP >> 8);
        UDS_READ_DID_FRAME.data.u8[3] = (uint8_t)PID_MIN_TEMP;
      ...
      case PID_TEMP_POINT_17:
        UDS_READ_DID_FRAME.data.u8[2] = (uint8_t)(poll_pid >> 8);
        UDS_READ_DID_FRAME.data.u8[3] = (uint8_t)poll_pid;
        poll_pid = poll_pid + 1;
        break;
      case PID_TEMP_POINT_18:
        ...
        break;
      case PID_MAX_CHARGE_VOLTAGE:
        UDS_READ_DID_FRAME.data.u8[2] = (uint8_t)(PID_MAX_CHARGE_VOLTAGE >> 8);
        UDS_READ_DID_FRAME.data.u8[3] = (uint8_t)PID_MAX_CHARGE_VOLTAGE;
        poll_pid = PID_MIN_DISCHARGE_VOLTAGE;
        break;
      case PID_MIN_DISCHARGE_VOLTAGE:
        UDS_READ_DID_FRAME.data.u8[2] = (uint8_t)(PID_MIN_DISCHARGE_VOLTAGE >> 8);
        UDS_READ_DID_FRAME.data.u8[3] = (uint8_t)PID_MIN_DISCHARGE_VOLTAGE;
        poll_pid = PID_ENERGY_COUNTERS;
        break;
      case PID_ENERGY_COUNTERS:
        UDS_READ_DID_FRAME.data.u8[2] = (uint8_t)(PID_ENERGY_COUNTERS >> 8);
        UDS_READ_DID_FRAME.data.u8[3] = (uint8_t)PID_ENERGY_COUNTERS;
        poll_pid = PID_ALLOWED_CHARGE_POWER;
        break;
      case PID_ALLOWED_CHARGE_POWER:
        UDS_READ_DID_FRAME.data.u8[2] = (uint8_t)(PID_ALLOWED_CHARGE_POWER >> 8);
        UDS_READ_DID_FRAME.data.u8[3] = (uint8_t)PID_ALLOWED_CHARGE_POWER;
        poll_pid = PID_ALLOWED_DISCHARGE_POWER;
        break;
      case PID_ALLOWED_DISCHARGE_POWER:
        UDS_READ_DID_FRAME.data.u8[2] = (uint8_t)(PID_ALLOWED_DISCHARGE_POWER >> 8);
        UDS_READ_DID_FRAME.data.u8[3] = (uint8_t)PID_ALLOWED_DISCHARGE_POWER;
      ...
      case PID_CELLVOLTAGE_CELL_1:
        UDS_READ_DID_FRAME.data.u8[2] = (uint8_t)(PID_CELLVOLTAGE_CELL_1 >> 8);
        UDS_READ_DID_FRAME.data.u8[3] = (uint8_t)PID_CELLVOLTAGE_CELL_1;
        poll_pid = PID_CELLVOLTAGE_CELL_2;
        break;
      case PID_CELLVOLTAGE_CELL_84:
        UDS_READ_DID_FRAME.data.u8[3] = (uint8_t)PID_CELLVOLTAGE_CELL_84;
        if (datalayer.battery.info.number_of_cells > 84) {
          if (nof_cells_determined) {
      ...
      case PID_CELLVOLTAGE_CELL_96:
        UDS_READ_DID_FRAME.data.u8[3] = (uint8_t)PID_CELLVOLTAGE_CELL_96;
        ...
        break;
      case PID_CELLVOLTAGE_CELL_108:
        UDS_READ_DID_FRAME.data.u8[3] = (uint8_t)PID_CELLVOLTAGE_CELL_108;
        poll_pid = PID_SOC;
        break;
      default:
    ...
    if (first_can_msg > 0 && currentMillis > first_can_msg + 1000) {
      transmit_can_frame(&UDS_READ_DID_FRAME);
    }
  }
