...
class MebBattery : public CanBattery, public IsoTp {
 ...
 protected:
  ...
  /* handle UDS response received via ISO-TP */
  ...
  static const int MAX_DTC_COUNT = CONFIG_ISOTP_MAX_MSG_LENGTH / 8;    // Maximum number of DTCs to store/display
  static constexpr unsigned long UDS_REQUEST_TIMEOUT_MS = 1000;  // Max wait for a UDS response before retrying
...
#define DCDC_MODE_CHARGE_12V 2    // buck — normal HV supply
  ...
  // off to the BMS for AC charging. Any fault collapses back to IDLE_HV_OFF.
  enum class HvCoordinatorState : uint8_t {
    IDLE_HV_OFF,         // command BMS HV_Off + DCDC Standby; wait for both to report it
    REQUEST_PRECHARGE,   // command DCDC PRECHARGE_ON; wait DC_IstModus_02 == PRECHARGE_ON
    PRECHARGING,         // precharge bit set; wait DC_Vorladung_Status complete AND HV voltage matched
    REQUEST_BMS_CHARGE,  // command BMS AC_Laden; wait BMS confirms (BMS_mode == AC_CHARGING)
    REQUEST_DCDC_BUCK,   // command DCDC CHARGE_12V; wait DC_IstModus_02 == CHARGE_12V
  ...
  uint8_t dcdc_actual_mode = DCDC_MODE_STANDBY;  // DC_IstModus_02 from DCDC_04
  bool dcdc_precharge_complete = false;          // DC_Vorladung_Status from DCDC_04
  int32_t dcdc_hv_voltage_dV = 0;                // DCDC_01 DC_IstSpannung_HV
  int16_t dcdc_hv_current_dA = 0;                // DCDC_01 DC_IstStrom_HV
  uint16_t dcdc_consumption = 0;                 // DCDC_02 DC_Verbrauch (display only)
  uint8_t dcdc_utilization_pct = 0;              // DCDC_02 DC_HYB_Auslastungsgrad (display only)
...
#define DC_FASTCHARGE_LS2 0xC0
  ...
  CAN_frame HVK_01_frame = {.FD = true,  //HVK_01
                            ...
                            .data = {0x5D, 0x61, 0x00, 0xFF, 0x7F, 0x80, 0x63, 0x03}};
